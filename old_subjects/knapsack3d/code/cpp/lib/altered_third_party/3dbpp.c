...
typedef struct {
  ...
  space   *binspace;     /* added by Henrique Becker              */
} allinfo;
...
void branch(allinfo *a, box *f, box *l, int miss, stype fill)
{
  ...
  int d;
  stype bound;
  /* added by Henrique Becker */
  ctype cp_ix; /* index of the current corner point */
  ctype cp_nb; /* the number of corner points */
  boolean at_least_one_placement;
  ...
  if (miss == 0) {
    ...
    /* The new bound below is the very worst bound there can ever exist.
     * It was added only to allow for running the code a first time and
     * checking if everything is ok. It needs to be improved after.
     * ~Henrique Becker*/
    bound = a->BVOL;
    ...
    if (bound > a->maxfill) {
      ...
      /*for (s = s1; s != sl+1; s++) {*/
      cp_nb = cpnb(a->binspace);
	for (i = f, d = 0; i != l+1; i++) {
	  if (i->k) continue; /* already chosen */
      at_least_one_placement = FALSE;
      for (cp_ix = 0; cp_ix < cp_nb; ++cp_ix) {

    /* Henrique Becker's Comment: The three tests below are replaced by
     * a call to can_be_placed. */
	  /* see if box fits at position s */
	  /*if ((int) (s->x) + i->w > a->W) continue;*/
	  /*if ((int) (s->y) + i->h > a->H) continue;*/
	  /*if ((int) (s->z) + i->d > a->D) continue;*/

    if (!can_be_placed(a->binspace, i->w, i->h, i->d, cp_ix)) continue;
    at_least_one_placement = TRUE;

    /* Henrique Becker's Comment: here the code will read from s
     * differently, as it will not be an array of points. Also,
     * the corner points are in base one. */
	  /* place box and call recursively */
	  /*i->k = TRUE; i->x = s->x; i->y = s->y; i->z = s->z; */
	  i->k = TRUE;
    i->x = cpx(a->binspace)[cp_ix] - 1;
    i->y = cpy(a->binspace)[cp_ix] - 1;
    i->z = cpz(a->binspace)[cp_ix] - 1;
    place(a->binspace, i->w, i->h, i->d, cp_ix);
    ...
    /* added by Henrique Becker
     * If some box type could not be placed in at least one corner point
     * at some call, then it is inutile to keep searching in this branch,
     * as placing boxes that can yet be placed will not create corner points
     * with more free space than the precious ones.*/
    if (!at_least_one_placement) return;
      } /* end of i for */
    }
  }
...
boolean onebin_robot(allinfo *a, box *f, box *l, boolean fast)
{
  ...
  a->binspace = new_space(a->W, a->H, a->D); /* added by Henrique Becker */
  ...
  del_space(a->binspace); /* added by Henrique Becker */
