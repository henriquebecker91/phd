...
it works well with the \emph{Plate-Size Normalisation} procedure we describe below;
...
The goal of the \emph{Plate-Size Normalisation} procedure we propose is to reduce the number of distinctly-sized plates considered.
...
The \emph{Plate-Size Normalisation} procedure we propose consists only of replacing every non-size-normalised plate enumerated by their normalised counterpart.
...
\begin{figure}[h]
  ...
  \center
  \input{diagrams/renormalisation_after_cut.tex}
...
\section{Adaptation to the rotation variant}
\label{sec:adaptation_for_rotation}

TODO: MOVE THIS TO EITHER THE INITIAL FORMULATION CHAPTER OR FOR THE MARTIN CHAPTER (Probably put this in the initial and mention it will be used in Martin and the adaptation chapters)
TODO: THIS DOES NOT INCLUDE THE MIRROR PLATE ROTATION YET
TODO: SAY WE TEST THE IMPACT OF MIRROR PLATE IN AN EXPERIMEN DOWN THE ROAD

To adapt the BBA formulation for the G2KP to the rotation G2KP, we need only to:
