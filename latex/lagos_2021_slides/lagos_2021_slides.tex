...
\usepackage{mathtools} % for mathbox
\usepackage{multirow}
...
\frame{\frametitle{Knapsack: Objective Function}
\center
...
\frame{\frametitle{Bin Packing: Objective Function}
\center
The Guillotine 2D Bin Packing Problem (G2BPP) minimizes the number of bins/plates necessary to obtain all pieces (by guillotine-cutting the plates).
% NOTE: we are dealing with the homogeneous variant in which all bin/plates have
% the same size.
...
% top left rectangle
%7p120x160
\fill[capri] ++(0, 0.255) +(0, 0) rectangle +(0.120, 0.160);
\draw[black] ++(0, 0.255) +(0,0) rectangle +(0.250, 0.250);

% top middle/right rectangle
%P250x250{ 2p114x118 8p66x148 }
\fill[creepers] ++(0.255, 0.255) ++(0.066,0) rectangle +(0.114, 0.118);
\fill[plum] ++(0.255, 0.255) +(0,0) rectangle +(0.066, 0.148);
\draw[black] ++(0.255, 0.255) +(0,0) rectangle +(0.250, 0.250);

% bottom left rectangle
%P250x250[
%  5p70x86
%  P250x164{ 4p83x140 1p167x184 }
%]
\fill[aquamarine] ++(0, 0) (0,0) rectangle +(0.083, 0.140);
\fill[french] ++(0, 0) (0,0.140) rectangle +(0.070, 0.086);
\fill[jam] ++(0, 0) (0.083,0) rectangle +(0.167, 0.184);
\draw[black] ++(0, 0) (0,0) rectangle +(0.250, 0.250);

% bottom middle rectangle
%P250x250{ 9p87x141 6p143x166 }
\fill[carmine] ++(0.255, 0) ++(0.143,0) rectangle +(0.087, 0.141);
\fill[dodger] ++(0.255, 0)  +(0,0) rectangle +(0.143, 0.166);
\draw[black] ++(0.255, 0) +(0,0) rectangle +(0.250, 0.250);

% bottom right rectangle
%P250x250{ 10p69x165 3p167x152 }
\fill[crimson] ++(0.510, 0) ++(0.167, 0) rectangle +(0.069, 0.165);
\fill[barbie] ++(0.510, 0) +(0, 0) rectangle +(0.167, 0.152);
\draw[black] ++(0.510, 0) +(0,0) rectangle +(0.250, 0.250);
...
\frame{\frametitle{Formulation: Knapsack}
\begin{align*}
\bm{\mathmakebox[0.7cm]{\textcolor{crimson}{max}}.} & \textcolor{crimson}{\sum_{(i, j) \in E} p_i e_{ij}}\\
\bm{s.t.} & \specialcell{\sum_{j \in E_{i*}} e_{ij} \textcolor{crimson}{\leq} u_i \hspace*{\fill} \forall i \in \bar{J},}\\
          & \specialcell{\sum_{o \in O}\sum_{q \in Q_{0o}} x^o_{q0} + \sum_{i \in E_{*0}} e_{i0} \leq \textcolor{crimson}{1} \hspace*{\fill},}\\
...
\frame{\frametitle{Formulation: Bin Packing}
\begin{align*}
% The phantom below is necessary to keep the same distance between the objective
% function and the constraints, avoiding unnecessary jiggling between slides.
\bm{\mathmakebox[0.7cm]{\textcolor{creepers}{min}}.} & \textcolor{creepers}{b} \phantom{\sum_{(i, j) \in E} p_i e_{ij}}\\
\bm{s.t.} & \specialcell{\sum_{j \in E_{i*}} e_{ij} \textcolor{creepers}{\geq} u_i \hspace*{\fill} \forall i \in \bar{J},}\\
          & \specialcell{\sum_{o \in O}\sum_{q \in Q_{0o}} x^o_{q0} + \sum_{i \in E_{*0}} e_{i0} \leq \textcolor{creepers}{b} \hspace*{\fill},}\\
          & \specialcell{\sum_{o \in O}\sum_{q \in Q_{jo}} x^o_{qj} + \sum_{i \in E_{*j}} e_{ij} \leq \sum_{k \in J}\sum_{o \in O}\sum_{q \in Q_{ko}} a^o_{qkj} x^o_{qk} \hspace*{0.05\textwidth} \forall j \in J, j \neq 0,}\\
\end{align*}
}
...
\small
\bibliography{lagos_2021_slides}
\end{frame}
\backupend
\end{document}
