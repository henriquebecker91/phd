...
\end{concern}
\begin{answer}
We cannot say if our paper contains enough novelty, as this is clearly up to the editors of MPC to decide.
We believe our contributions are well presented in the paper.
We have tracked down and presented the closest ideas to the ones we have employed, as we believe this is part of the scientific process and shows intellectual honesty.
However, maybe this has given the false impression the work of perceiving the potential, the possibility, and how exactly to integrate these ideas on another context (none of them were applied to a mathematical formulation), was a work devoid of innovation.
Even if this was the case, the knowledge of the extent these enhancements improve the performance of the formulation is novel.
\end{answer}
...
\end{concern}
\begin{answer}\label{ans:make_it_self_contained}
\autoref{con:ex1_and_more_diagrams} asks for more diagrams, which we provided. We hope they help to improve the clarity of the basic problem definitions.

The other two referees have brought up concerns about the lack of an explanation about the pricing procedure (see \autoref{con:describe_pricing} and \autoref{con:describe_pricing_2}), described in~\cite{furini:2016}. We address these three concerns together here with the following addition:

\newtext{NEW SECTION: The pricing phase}
...
For us, it seems clear the empirical results, especially about the 80 harder instances from~\cite{velasco:2019}, are of interest to the community (which is not so narrow to not have a recent and sizeable survey of its own, see~\cite{russo:2020}).
...
\begin{concern}
Example 1 and the basic problem definitions could be better explained with diagrams.\label{con:ex1_and_more_diagrams}
\end{concern}
\begin{answer}
...
\end{concern}
\begin{answer}
We decided to remove this part of our conclusions, but here we explain our previous statement. The Gurobi's barrier method was run multithreaded but it had some drawbacks: (i) for deterministic runs, as it is our case, Gurobi reports a `Concurrent spin time' which indicates the time lost by the threads because they are blocked by other threads; (ii) nor the initial ordering, nor the crossover step seem to benefit from multiple threads. This concern has also lead us to notice that in the paper we wrongly stated the parallelisation was one thread per LP solving method (an old behavior of the solver), when it in fact uses one thread for each LP method method besides barrier, and all remaining threads for barrier. This was corrected.
\end{answer}
...
The way both formulations work also makes this process to be not straightforward.
...
\end{concern}
\begin{answer}
We have removed the offending paragraph.
\end{answer}
...
\end{concern}
\begin{answer}
The same was suggested in~\autoref{con:ex1_and_more_diagrams} and this suggestion is addressed there.
\end{answer}
...
\end{concern}
\begin{answer}
We thank the referee for the heads up on the need to explain the benefits of MILP techniques.
To be honest, we do not know the readership of MPC well enough to have made this decision on our own.
We decided to keep only the first sentence of the mentioned paragraph.

The referee guessed right, there is little to no difference in the value of the relaxation between the formulations, we felt this was expected but truly we should have mentioned it explicitly.
We did include the following sentence in the conclusions, where we attribute the gains of our model to symmetry-breaking:
...
\node [below] at (\labelxshift, \labelyshift) {\labelfontsize Item 2a};
...
\node [below] at (\labelxshift, \labelyshift) {\labelfontsize Item 2(b)ii (2x7 piece not exists)};
...
\draw (0, 0) rectangle +(15, 2) node [midway] {waste}; % [pattern=north west lines]

\draw (0, 3) rectangle +(12, 12) node [midway] {\innerlabelfontsize 12x12};
\end{scope} % SECOND IMAGE

\node [below] at (\labelxshift, \labelyshift) {\labelfontsize Item 2(b)ii (2x7 piece exists)};
\end{scope} % FIRST PAIR
...
\node [below] at (\labelxshift, \labelyshift) {\labelfontsize Item 2(b)iii};
