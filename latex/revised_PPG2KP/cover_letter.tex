...
\end{concern}
\begin{answer}
We do not address this concern here because the referee further break it down to more detailed concerns about the code and we address each of them individually. We do believe the code will now be reusable and useful for the community without our help. The detailed concerns which we address follow: \autoref{con:minimal_testing}, \ref{con:no_readme}, \ref{con:no_tutorial}, \ref{con:get_cut_pattern_not_understood}, and \ref{con:abandoned_code}.
\end{answer}
...
\end{concern}
\begin{answer}
TODO: Get a diagram for Example 1, and get the thesis proposal diagrams (maybe a merge or adaptation of them) for the problem.
\end{answer}
...
\end{concern}
\begin{answer}
TODO: Do as asked, remove section 5.2 and replace it by a few sentences.
\end{answer}
...
\end{concern}
\begin{answer}
TODO: move the explanation to the table caption. Unfortunately the referee did not say what are the standard abbreviations, so we could just replace our column names. So we need to find these standard abbreviations if we want to use them.
\end{answer}
...
\end{concern}
\begin{answer}
TODO: just fix them all; check last one just to be sure.
\end{answer}
...
\end{concern}
\begin{answer}
TODO: something in the following lines: The correctness of the results comes from the fact multiple instances employed were previously optimally solved in the prior work and these results match; also, for the base formulation from Furini we obtain the number of variables down to the exact number. Unit tests are not so useful because the most natural way to validate any reduction is either by mathematical proof (as presented in this paper) or by obtaining optimal results for a large amount of instances (integration testing).
\end{answer}
...
\end{concern}
\begin{answer}
TODO: improve the readme: link the already existing documentation and give a tutorial on how to call the main script (which already has a \inlinecode{--help} flag).
\end{answer}
...
\end{concern}
\begin{answer}
TODO: provide these; probably in the README.
\end{answer}
...
The function in question starts with a 17-line comment explaining how it works, and then have more comments further. \emph{We have further improved its comments}\footnote{TODO: improve the comments and link them here.}, but we found necessary to point out that it is a complex procedure at its core, and making it trivial to follow is not a reasonable goal.
...
\end{concern}
\begin{answer}
We apologize for this oversight. The code was downloaded by means of the git repository. We do not save the raw data in the git repository because of the sheer volume of it. The raw data yet exists and we have uploaded it to the following link: TODO: upload the raw data to a Google Drive archive.
\end{answer}
...
\end{concern}
\begin{answer}
We apologize for the bad experience. The commit in which we applied these suggestions can be viewed here: TODO: make this change in a commit only for it, and post the link here.
\end{answer}
...
\end{concern}
\begin{answer}
TODO: rewrite the phrase, it was critisized by Prof. Ritt too, I can expand it a little and make it more intelligible.
\end{answer}
...
\end{concern}
\begin{answer}
TODO: explain it better, something in the following lines: A single series of parallel cuts over the original plate rarely leads to the pieces. The pieces are obtained by cuts made over smaller plates obtained by previous guillotine cuts.
\end{answer}

\begin{concern}\label{con:constrained_NP_hard_first}
In the paragraph which starts with `Constrained demand means [...]', the description is not clear since, clearly, an upper bound on the number of copies \(u_j\) can be always set. First of all I think you wanted to say `strongly' NP-Hard (the problem clearly remains difficult also with unconstrained demands). For instance, just considering the area bound, \(u_j = \lfloor \frac{L \cdot W}{l_j \cdot w_j} \rfloor\). Is this bound better that the one you present, i.e.,\(u_j < \lceil L / l_j \rceil \cdot \lceil W / w_j \rceil\) (by the way, where does this bound come from?). Consider for instance \(L = W = 10\) and one piece~\(j\) with \(l_j = 9\) and \(w_j = 1\), the two upper bounds are different I think, and probably even stronger bounds can be computed.
\end{concern}
\begin{answer}
TODO: fix the NP-Hard tidbit (it should be `strongly'), second the area bound was incorrect, we meant to use `\(\lfloor\)' not `\(\lceil\)' (which works for the variant without rotation); finally explain that instances with demand below this bound are interesting because what makes the problem harder (strongly NP-Hard instead of weakly NP-Hard) is the fact the solution method cannot assume any plate of the same size as a piece can be sold as a piece, i.e., it becomes necessary to keep track of which pieces are yet interesting to produce, making DP-based solutions suffer with a exponential growth in the number of states.
\end{answer}
...
\end{concern}
\begin{answer}
TODO: we can use the pictures from the thesis proposal presentation, but it will not be a single picture.
\end{answer}
...
\end{concern}
\begin{answer}
TODO: `you saying that there are mistakes in these papers?' Yes, and this is the expression used by the authors of the survey. `we do not compare against the formulations proposed in [19, 21, 22]' Since we have submitted the paper, we have contacted the authors of the mentioned formulations, and recently submitted a conference paper making such comparison, so now we do not want to risk using material under evaluation elsewhere.
\end{answer}
...
\end{concern}
\begin{answer}
TODO: the section 5.2 (about the choice of LP algorithm) can be replaced by a paragraph. The ideas about the pricing procedure will be addressed in a previous concern.
\end{answer}
...
\end{concern}
\begin{answer}
TODO: probably we can remove mention to purge, but I will mention here in this concer that purge `worked', the purpose of purge was not to improve the performance, but instead to have a better notion on how many variables are non-redundant.
\end{answer}
...
\end{concern}
\begin{answer}
Our intention was to say `is not strongly NP-hard;', which was correctly guessed in~\autoref{con:constrained_NP_hard_first}. This mistake is fixed.
\end{answer}
...
\end{concern}
\begin{answer}
The order was switched. TODO: switch the order in the paper.
\end{answer}
...
\end{concern}
\begin{answer}
You are correct. This imprecision was already present~\cite{furini:2016} and was reverberated here. The code deals with this correctly (i.e., it is equal to two when the two child plates are equal). The text was reworked. TODO: rework the text and copy it below.
\end{answer}
...
\end{concern}
\begin{answer}
The lack of a condensed explanation of the pricing procedure was also noted in~\autoref{con:describe_pricing}. Also, \autoref{con:not_self_contained} asks for a more self-contained paper, which would include more information on Furini's model and the pricing procedure. The text added to address these three concerns can be found in~\autoref{ans:make_it_self_contained}.
\end{answer}
